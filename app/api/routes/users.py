...
async def register_user(
    ...
    with map_http_exceptions((ValueError, 400)):
        result = await auth_service.RegisterUser(email=payload.email, password=payload.password)
...
async def login_user(
    ...
    with map_http_exceptions((PermissionError, 403), (ValueError, 400)):
        result = await auth_service.LoginUser(email=payload.email, password=payload.password)
...
async def refresh_access_token(
    ...
    with map_http_exceptions((PermissionError, 403), (ValueError, 401)):
        result = await auth_service.RefreshAccessToken(refresh_token=payload.refresh_token)
        return AccessTokenResponse(**result)


@router.get("/me")
async def get_service_user_profile(current_user: User = Depends(get_current_user)) -> UserProfileResponse:
    """获取当前登录用户信息。"""
